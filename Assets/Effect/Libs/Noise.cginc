...
float4 grad4(float j, float4 ip)
  ...
  //p.xyz = floor( frac (float3(j) * ip.xyz) * 7.0) * ip.z - 1.0; // Error?
...
// http://prideout.net/blog/?p=63
  ...
  float curlX(float3 v, float d){
  	return ( (snoise3D(float3(v.x,v.y+d,v.z)).z - snoise3D(float3(v.x,v.y-d,v.z)).z) - (snoise3D(float3(v.x,v.y,v.z+d)).y - snoise3D(float3(v.x,v.y,v.z-d)).y) ) /2/d;
  }
  
  float curlY(float3 v, float d){
  	return ( (snoise3D(float3(v.x,v.y,v.z+d)).x - snoise3D(float3(v.x,v.y,v.z-d)).x) - (snoise3D(float3(v.x+d,v.y,v.z)).z - snoise3D(float3(v.x-d,v.y,v.z)).z) ) /2/d;
  }
  
  float curlZ(float3 v, float d){
  	return ( (snoise3D(float3(v.x+d,v.y,v.z)).y - snoise3D(float3(v.x-d,v.y,v.z)).y) - (snoise3D(float3(v.x,v.y+d,v.z)).x - snoise3D(float3(v.x,v.y-d,v.z)).x) ) /2/d;
  }
